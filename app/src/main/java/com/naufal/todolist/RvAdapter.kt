...
class RvAdapter(private val listener: (TodoModel, Int) -> Unit): RecyclerView.Adapter<RecyclerView.ViewHolder>(), Filterable {
    private val VIEW_EMPTY_DATA = 0
    private val VIEW_TODO = 1

    private var todoList = listOf<TodoModel>()
    private var todoSearchResult = listOf<TodoModel>()

    fun setTodoList(todoList: List<TodoModel>) {
    ...
    override fun getFilter(): Filter {
        return object : Filter() {
            ...
            override fun publishResults(constraint: CharSequence?, results: FilterResults?) {
                todoSearchResult = results?.values as List<TodoModel>
                notifyDataSetChanged()
            }
        }
    }
        ...
        when (holder.itemViewType) {
            ...
            VIEW_TODO -> {
                ...
                val sortedList = todoSearchResult.sortedWith(
                    if(MainActivity.urutBerdasarkanDateCreated)
    ...
    class TodoViewHolder (itemView: View): RecyclerView.ViewHolder(itemView){
        fun bindItem(todo: TodoModel, listener: (TodoModel, Int) -> Unit) {
            ...
            val date = if (todo.dateUpdated != todo.dateCreated) "$dateUpdated" else "$dateCreated"
            ...
            val dueDateTime = "Tenggat ${dueDate}, ${todo.dueTime}"
